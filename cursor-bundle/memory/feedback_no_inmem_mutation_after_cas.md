...
**How to apply:** Treat any `setX(...)` on a CAS-managed entity (`loan_account.disbursement_status` / `loan_status`, `loan_account_events_queue.data->>'disbursement_status'`) as a bug — fail loud in review. If a value must change, route it through the state-machine CAS or `patchJsonFields`, not a setter. Use the **`state-machine-safety`** skill before any state-column change. .cursorrules §0 Rule 3. Pairs with [[feedback_concurrency_contract_audit]], [[rca-workflow]].
