...
4. **Mixed trains** — Do not run `bash scripts/bin/sync-branches.sh --train <branch> --yes` across all repos unless the user explicitly wants one train everywhere. Use [`mixed-train-matrix.md`](../brain/runbooks/mixed-train-matrix.md): DFC→accounting `3.7.1`; disburse RCA on `3.4.2.2`→scoped LOS+accounting; then `kg-switch.sh`.
