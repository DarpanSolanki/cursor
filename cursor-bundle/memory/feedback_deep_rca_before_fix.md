...
1. **Code-back every step** — trace the exact execution path with `txn-graph` / `kg flow` / file:line reads; cite `file:line` for each claim (proof-backed gates in CLAUDE.md §0.7). No half-recall.
