...
description: "Build green before every push, and \"built green\" is necessary but NOT sufficient for \"done\". Build via `cd <repo> && ./gradlew build -x test` (Java 17). Status stays \"pushed; awaiting QA retest\" until QA confirms — never say fixed/completed/done before that."
metadata: 
  node_type: memory
  type: feedback
...
**Build green before every push.** Run `cd <repo> && ./gradlew build -x test` (Java 17) and confirm it passes before pushing. For state-machine / CAS / posting changes, also paste the expected/actual **DB-state delta** (main-vs-branch) — "built green" is necessary, not sufficient.
