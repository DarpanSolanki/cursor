...
**Why:** the local checkout is frequently on a different/stale branch than what the env actually runs. In the 2624020 disbursement RCA, the working tree was on `feature/dpic-v1`, the user first said `mfi_release_v3.5.0`, then the real deployed branch was `feature/delayed_payment_interest` — only on `upstream` (khoslalabs), not `origin`. Mapping the stack trace against the wrong ref gives wrong line numbers and wrong conclusions.
