...
> **This doc covers the TRIGGER half** (how a *registered* job's cron fires: `@Scheduled` + the batch-service `AutoScheduler` arming `batch_schedule` rows). For the **REGISTRATION half** — how a job's `batch_schedule` row comes to exist (`Loader.initJobs @PostConstruct → *JobLoader.loadJobs → BatchConfigService.buildJobForTenant → setUpJobAdvanceV2` auto-seeds `batch_job`/`batch_schedule`), the `BatchJobPlaceholderConfig` stub bean, and the `api_master` routing row — see [`system-activation-and-wiring.md`](system-activation-and-wiring.md) §1. A job that is never registered has **no `batch_schedule` row to arm** → fires nowhere, silently.
