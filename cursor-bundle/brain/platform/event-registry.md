...
- **Producer (best-effort, literal reference):**
  - `./novopay-platform-accounting-v2/src/main/java/in/novopay/accounting/batchnew/loanrecurringpaymentbatchapi/LoanRecurringPaymentItemWriter.java`:203 → `accountingKafkaProducer.pushDataToKafkaQueue(jsonObject.toString(), "bulk_collection_data_");`
- **Consumer:** service `novopay-platform-payments` | class `in.novopay.payments.collections.mfi.consumer.CreateOrUpdateBulkCollectionConsumer` | methods ``computeRecords`, `processCollectionData`, `processCollectionForAllocation``
...
- **Producer (best-effort, literal reference):**
  - `./novopay-platform-payments/src/main/java/in/novopay/payments/collections/mfi/util/BulkCollectionExtractor.java`:52 → `private static final String BULK_COLLECTION_FAILED_RECORDS_TOPIC = "bulk_collection_data_failed_";`
- **Consumer:** service `novopay-platform-accounting-v2` | class `in.novopay.accounting.loan.recurring.entity.BulkCollectionFailedRecordConsumer` | methods ``computeRecords``
...
- **Producer (best-effort, literal reference):**
  - `./novopay-platform-lib/infra-message-broker/src/main/java/in/novopay/infra/message/broker/core/NovopayMessageBrokerBuilder.java`:349 → `stringBuilder.append("disburse_loan_api_");`
  - `./novopay-mfi-los/src/main/java/in/novopay/los/util/DisburseLoanAPIUtil.java`:83 → `losMessageKafkaProducer.pushDataToKafkaQueue(request, "disburse_loan_api_");`
- **Consumer:** service `novopay-platform-accounting-v2` | class `in.novopay.accounting.consumers.LmsMessageBrokerConsumer` | methods ``computeRecords`, `processConsumerRecord``
...
- **Producer (best-effort, literal reference):**
  - `./novopay-platform-accounting-v2/src/main/java/in/novopay/accounting/loan/constants/LMSLOSSyncConstants.java`:14 → `public static final String LOS_LMS_SYNC_TOPIC = "los_lms_data_sync_";`
- **Consumer:** service `novopay-mfi-los` | class `in.novopay.los.kafka.LmsDataSyncConsumer` | methods ``computeRecords``
...
- **Producer (best-effort, literal reference):**
  - `./novopay-platform-accounting-v2/src/main/java/in/novopay/accounting/consumers/LmsMessageBrokerConsumer.java`:209 → `accountingKafkaProducer.pushDataToKafkaQueue(messageJson, "los_lms_disbursement_sync");`
- **Consumer:** service `novopay-mfi-los` | class `in.novopay.los.kafka.DisbursementSyncConsumer` | methods ``computeRecords``
