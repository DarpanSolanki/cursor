...
- 2026-07-20 | BUG_FIX (uncommitted) | TDPQA-54 disbursement Redis in-flight locks: LOS producer and Accounting consumer now use atomic owner-token acquire with configurable 600000 ms TTL; platform-lib adds Lua compare-and-delete; ambiguous intermediate `DEFAULT` replay fails closed. Builds PASS; `disbursement.redis_inflight_lock_sim` PROCESSOR_MIRROR_SIM + LOCAL_REDIS_RUNTIME PASS; live INDL fixture blocked before loan creation by existing mandate validation. | `trustt-platform-lib/infra-cache`, `trustt-platform-los/.../DisburseLoanAPIUtil.java`, `trustt-platform-accounting/.../LmsMessageBrokerConsumer.java`, `scripts/testing/registry.json`
