...
| **RESOLVED (2026-07-08) — DPI grace kill-switch / missing overdue segment boundary** | **Resolved (was High)** | `DpiAccrualCalculationBatchService.precomputeDaySnapshots` admits base/anchor only after `computeOverdueDate` (due+grace+1); `buildSegmentBoundaries` splits on overdue days as well as due days (`46f115199`); harness `dpic.grace_overlap_e2e` + `dpic.multi_emi_installment_e2e` on LAN 8057160 | When EMI2 still in grace, day-0 door of multi-day slice kept EMI1 base forever past EMI2 overdue — or zeroed older-EMI DPI if latest-EMI grace gated the whole day. |
