...
| **GAP-082 — SHG INT distribute ₹1: (a) per-segment parent≠Σchildren **FIXED 2026-08-03**, (b) child accrued≠child scheduled INT **FIXED 2026-08-03**** | **(a) FIXED** · **(b) FIXED** — child now reads its OWN persisted `loan_due_details` INT on a fully-accrued window (RCA: CLB derives child INT as `EMI − PRIN` then rebalances the whole schedule, so it is not a split of the parent's INT and cannot be re-derived from it). Closed-member residue stays on the parent. Guard: `iad_column_audit.closed_window_accrued_eq_own_scheduled_int`. | `trustt-platform-accounting/src/main/java/in/novopay/accounting/loan/grouploan/interest/service/InterestGroupLoanAccrualDistributionService.java` L282-L295 (`mirrorParentSegments`: per-child HALF_UP split, last segment absorbs remainder, no cross-child reconciliation to the parent segment). Local REAL-job evidence 2026-08-03 on `mfi_integration_v3.4.2.4`, fresh SHG parent 6004162825: segment 2024-08-21..08-31 parent=306 children=184+123=**307**; segment 08-31..09-24 parent=702 children=420+281=**701**; child 6004163026 accrued 604 vs scheduled INT 605; child 6004163027 accrued 404 vs scheduled INT 403. Caught by new `flowtest.fresh_loan_int_accrual_e2e` sections K2/K3. **Provenance note:** original evidence came from a stale JVM (started 15:52, classes 16:02, HEAD `ad399c5f2` committed 16:03). **Re-run 2026-08-03 after rebuild+restart on `ad399c5f2d` reproduces identical values** (307/306, 701/702, 604/605, 404/403) — the fix does NOT address this gap. Stale-runtime root cause fixed: `aops_java_newer_than_boot` compared source mtime to the boot LOG the running service appends to, so the check could never fire; `novopay-service.sh ensure` also ignored `--compile` on a green probe. New gate `scripts/bin/assert-build-current.sh`. **Cadence:** reproduces under daily (34 fires) AND under `hop` (4 fires, +1 intermediate calc per gap) — it needs repeated re-distribution of the open segment, not every calendar day; posting-days-only (2 fires) under-fires and PASSES, hiding it. **Cadence-dependent:** the per-segment break (K2) reproduces only when the chain is fired **daily** (production EOD cadence) — every calc re-distributes the open segment and the rounding accumulates. Firing only month-end+due days does 2 distributes instead of 34 and K2 PASSES, so sparse-firing harnesses hide it. K3 (child accrued vs child scheduled INT) is cadence-independent. | Errors cancel over the installment window and across the group, so `verify_shg_interest_accrual_parity.sql` returns **PASS (diff 0.000000)** — a month-end parent-vs-children GL/reporting cut carries a ₹1 break, and each child's accrual ledger differs from the interest billing actually bills it (billing uses the child schedule). Related to GAP-080 (schedule drift) but distinct: this is the accrual distribute split.

**(a) FIXED 2026-08-03 (local, not pushed):** `mirrorParentSegments` now pins posted segments and splits only the unposted remainder — was `segShare.max(posted)`, which let a re-split raise an already-POSTED segment (child0 183→184 on the 09-12 calc: 385×306/642 = 183.504 → HALF_UP 184). Verified red→green: per-segment asserts delta 0.000000; JLG 24/24 no regression. **(b) still open:** child accrued 604 vs own scheduled INT 605 — distribute uses trunc(parent_rounded_total × fraction)+carry-to-last, child schedule uses its own principal (30000×22%/360×33 = 605.00). **Same (a) defect exists unfixed on 3.7.1 DPI** — `DpiGroupLoanAccrualDistributionService` L404 + L388-389. |
