...
def _resolve_roll_window(parent_id: int) -> tuple[date, date, set[date]]:
    last_iad_s = psql(
        f"""
...
"""
    ...
    next_due_s = psql(
        f"""
SELECT MIN(due_date)::date::text
FROM mfi_accounting.loan_due_details
WHERE loan_account_id={parent_id}
  AND COALESCE(is_deleted,false)=false
  AND due_date > DATE '{last_iad.isoformat()}'
"""
    )
    if not next_due_s:
        raise RuntimeError(
            f"no next due after last_iad={last_iad} for parent account_id={parent_id}"
        )
    next_due = date.fromisoformat(next_due_s)
...
WHERE loan_account_id={parent_id}
  AND COALESCE(is_deleted,false)=false
  AND due_date BETWEEN DATE '{start.isoformat()}' AND DATE '{end.isoformat()}'
"""
    ).strip()
    dues = {date.fromisoformat(x.strip()) for x in dues_raw.splitlines() if x.strip()}
...
def main() -> int:
    ...
    start, end, dues = _resolve_roll_window(parent_id)
    ...
    print(
        ...
        f"posting_days={len(posting_days)}"
    )
