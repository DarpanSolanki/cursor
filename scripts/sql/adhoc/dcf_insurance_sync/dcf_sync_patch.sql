...
SELECT COUNT(*) AS patch_b_count FROM ops_dcf_sync_b;
...
BEGIN
    ...
    IF c <> 4 THEN
        RAISE EXCEPTION 'patch_b_count % expected 4', c;
    END IF;
END $b$;
...
WHERE t.id = e.task_id AND COALESCE(t.is_deleted, false) = false;
...
SET
    ...
    updated_by   = 'OPS_DCF_SYNC'
FROM ops_dcf_sync_b e
WHERE s.id = e.staging_id AND COALESCE(s.is_deleted, false) = false;
...
SELECT COUNT(*) AS patch_a_count FROM ops_dcf_sync_a;
...
BEGIN
    ...
    IF c <> 101 THEN
        RAISE EXCEPTION 'patch_a_count % expected 101', c;
    END IF;
END $a$;
