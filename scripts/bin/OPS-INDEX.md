...
|------|---------|-----------|
| `accounting-flow-coverage.sh` | Accounting flow coverage report — ALL domains (read, write, batch, money). | scripts/bin/accounting-flow-proof.sh, scripts/lib/test_domain_coverage_gate.py, scripts/testing/agent_router.py |
...
| `db-local-write.sh` | Local Yugabyte writes only (127.0.0.1:5433). Agents use this instead of raw psql to remote | scripts/bin/disburse-indl-kafka-quick.sh, scripts/bin/disburse-indl-quick.sh, scripts/bin/purge-local-dpi.sh, scripts/lib/local_parity_gate.py, scripts/lib/test_workspace_contract.py |
...
| `hot-path-scan.sh` | Workspace hot-path perf heuristic (DAO-in-loop, stream-in-loop). Agents only. | scripts/bin/ship-loop-gate.sh, scripts/testing/agent_router.py, scripts/testing/workspace_autopilot.py |
| `impact-tests.sh` | Dynamic impact-tests — git diff → KG blast radius → registry cases + WHY. | .cursor/hooks/kg-session-watermark.sh, scripts/bin/ship-loop-gate.sh, scripts/lib/pipeline_chain_audit.py, scripts/lib/rules_to_gates.py |
| `initial-setup-local.sh` | (no header) | — |
| `install-kg-git-hooks.sh` | Install post-checkout hook in each service repo → kg-session-sync on branch change. | scripts/bin/install-user-cursor-gates.sh, scripts/bin/new-machine-setup.sh |
| `install-user-cursor-gates.sh` | Install / verify Cursor hooks + git gates for sliProd. | scripts/bin/new-machine-setup.sh, scripts/bin/workspace-max-pass.sh |
...
| `learn.sh` | Capture a durable fact into cursor-bundle/memory + MEMORY.md in one command. | .cursor/hooks/stop-ship-nudge.sh, scripts/bin/test-learn.sh, scripts/bin/workspace-sanity.sh, scripts/lib/knowledge_miss.py, scripts/testing/intelligence_hub.py, scripts/testing/learn_cli.py |
...
| `payments-flow-coverage.sh` | Payments flow coverage report — ALL domains (read, write, batch, money). | — |
...
| `schema-sync.sh` | Refresh the schema oracle: structure (local DB) → code binding (Java) → train labels (Flyw | scripts/bin/new-machine-setup.sh, scripts/lib/column_binding.py, scripts/lib/schema_live_drift.py, scripts/lib/sql_column_check.py, scripts/lib/test_sql_column_check.py |
...
| `train-delta.sh` | What code is actually new between two trains — the "is this new to production?" question. | scripts/lib/test_agent_router.py, scripts/testing/agent_router.py |
| `train-upstream-sync.sh` | Ensure local/origin train tip is not behind upstream before mainline push. | scripts/bin/push-origin.sh, scripts/lib/rules_to_gates.py |
| `work-monitor.sh` | What is running right now — agents, batch jobs, builds, tests, and the DB they share. | — |
