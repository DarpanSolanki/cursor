#!/usr/bin/env bash
# QA1 SDCP-10497 regression: 15th-EMI go-live + month-boundary accrual posting.
# Replays QA shape — one calc + booking per calendar day (BOD after each EOD), not booking on due days only.
set -euo pipefail
...
echo "=== DPI posting calendar QA regression loan=$LOAN_ACCOUNT_ID go_live=$GO_LIVE_ISO end=$END_DATE ==="
...
echo ">>> QA-style daily EOD: calc + booking each calendar day $GO_LIVE_ISO .. $END_DATE"
while IFS= read -r day; do
  ...
  echo "    EOD $day — calc + booking"
  call_batch dpiAccrualCalculation "$ms"
  call_batch dpiAccrualBooking "$ms"
done <<<"$CALENDAR_DAYS"

read -r accrual_rows <<<"$(
  dpi_pg -t -A -c "SELECT COUNT(*) FROM mfi_accounting.dpi_accrual_details WHERE loan_account_id=$LOAN_ACCOUNT_ID AND is_deleted=false AND total_accrued_amount>0;"
)"
[[ "${accrual_rows:-0}" -gt 0 ]] || fail "expected accrual rows after calc, got ${accrual_rows:-0}"

FINAL_MS="$(date_to_ms "$END_DATE")"
echo ">>> billing on final EOD $END_DATE"
...
[[ "${unposted:-1}" == "0" ]] || fail "posting gaps: unposted=$unposted posted=$posted closed=$closed"
...
[[ "${booked:-0}" -gt 0 ]] || fail "expected booked_rows > 0, got booked=$booked"
[[ "${billed:-0}" -gt 0 ]] || fail "expected billed_rows > 0, got billed=$billed"

echo "OK posting calendar posted=$posted booked=$booked billed=$billed"
echo "PASS: DPI posting calendar QA regression"
