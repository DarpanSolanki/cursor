-- Shift demo loan installments 6–8 to 1st-of-month EMI (anchor-at-segStart regression).
\set ON_ERROR_STOP on

BEGIN;

CREATE TABLE IF NOT EXISTS mfi_accounting._dpi_emi_first_backup (
...
INSERT INTO mfi_accounting._dpi_emi_first_backup (
...
WHERE lid.loan_account_id = :loan_account_id::bigint
  AND lid.is_deleted = false AND lid.serial_number IN (6, 7, 8)
...
WHERE ldd.loan_account_id = :loan_account_id::bigint AND ldd.is_deleted = false
...
SET installment_date = CASE lid.serial_number
        WHEN 6 THEN TIMESTAMP '2026-05-01 00:00:00'
        WHEN 7 THEN TIMESTAMP '2026-06-01 00:00:00'
        WHEN 8 THEN TIMESTAMP '2026-07-01 00:00:00'
      END,
    ...
    is_settled = false,
    updated_on = NOW(),
    updated_by = 'DPI_EMI_FIRST_FIXTURE'
WHERE lid.loan_account_id = :loan_account_id::bigint
  AND lid.is_deleted = false AND lid.serial_number IN (6, 7, 8);
...
WHERE ldd.loan_account_id = :loan_account_id::bigint AND ldd.is_deleted = false
  ...
  AND lid.serial_number IN (6, 7, 8) AND lid.is_deleted = false;
