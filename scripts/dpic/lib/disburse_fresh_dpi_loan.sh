...
eval "$(python3 "$ROOT/scripts/dpic/demo/lib/compute_dates.py" --anchor "$ANCHOR_DATE")"

TS="$(date +%s%3N)"
EXT_REF="${EXT_REF:-DPIC_CERT_${TAG}_${ANCHOR_DATE//-/}_${TS}}"
CUSTOMER_ID="${CUSTOMER_ID:-$((10000000 + TS % 8999999))}"
REQUEST_SCRATCH="$ROOT/scripts/scratch/dpic_cert_${TAG}_${TS}.json"
...
python3 "$ROOT/scripts/dpic/demo/lib/validate_disburse_dates.py" "$DEMO_DISBURSE_MS" "$DEMO_FIRST_EMI_MS"
